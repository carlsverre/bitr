...
// MIT License
//
// all serializing modifications by carlsverre, carl@carlsverre.com
...
function lookupOrCreate(req,opts){
  ...
  if(ownProp(sessions,id)){
    session=sessions[id];
    if(DEBUG) debug("SESSION: found session",inspect(session));
  }
  else{
    if(DEBUG) debug("SESSION: creating new session");
